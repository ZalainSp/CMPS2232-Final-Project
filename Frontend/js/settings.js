document.addEventListener("DOMContentLoaded", () => {
  // Set the Dashboard link to point to the correct dashboard for the logged-in user's role
  try {
    ...
    if (user && user.role) {
      ...
      const correctPage = roleToPage[role] || "dashboard-customer.html";

      // Update the Dashboard link
      const dashboardLink = document.getElementById("dashboard-link");
      if (dashboardLink) {
        dashboardLink.href = correctPage;
      }
    }
  } catch (e) {
    // ignore; fallback to customer dashboard
  }
